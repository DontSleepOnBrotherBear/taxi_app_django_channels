...
class TestWebSocket:
    ...
    async def test_request_trip(self, settings):    #When a rider requests a trip, the server will create a new Trip record and will broadcast the request to the driver pool. But from the rider's perspective, they will only get a message back confirming the creation of a new trip. That's what this test does.
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
